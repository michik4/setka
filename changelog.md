...
> `{релиз версия}`.`{версия с дополнениями}`.`{fix версия}` `{буквенные версии}` <br>
> масштаб обновления идет слева направо от самых значительных до минимальных <br>
> - релиз версия - версии прошедшие все этапы разработки и тестирования
> - версия с дополнениями - добавление функций меняющий пользовательский опыт
> - fix версия - исправление багов и недочетов UI/UX
> - буквенные версии - добавление или исправление вещей косвенно касающихся проекта

## 0.5.1 

### New features (minor)
- Добавлена ссылка на название альбома фото
- Добавлена вариация страницы музыки отображающая треки других пользователей
- Музыкальные альбомы 
  - Добавлена пагинация на странице муз. альбома 
  - Добавлены сортировки на странице муз. альбома (название по алф., исполнитель по алф., длительность)
- Добавлена возможность перелистывать фотографии на странице "Фотографии"
- Добавлено соотношение сторон у плейсхолдера удаленной фотографии
- Добавлена возможность менять очередность фотографий в постах (при создании и редактировании)
- Добавлено ограничение высоты постов в 600px после чего остаточная часть будет скрываться градиентом.

### UI/UX Fix
- Исправлена кнопка удаления фотографий в форме редактирования и создания поста (пропадала за границами блока)
- Исправлено отображение всего 3 треков в альбоме (из-за max-height)
- поле ввода в форме создания поста теперь имеет автоматический размер взависимости от кол-ва символов

### Bugfix
- Исправлена ошибка из-за которой у пользователя без прав в сообществе появляется форма создания поста от лица пользователя (не на стену сообщества)
- Исправлены ошибки связанные с динамическими плейсхолдерами удаленных фотографий
  - На сервере плейсхолдеры не удалялись после отключения пользователя

### Other 
- добавлены `vseti_scripts` для работы с проектом
  - `create-migration.py` скрипт для создания дампа схемы базы данных в формате миграции (можно использовать для полного создания бд или добавления недостающих таблиц или столбцов таблиц)

## 0.5.0
> dev time 02.05.2025 - 13.05.2025

### New Features
- Полностью переработана сетка фотографий (PhotoGrid)
  - Теперь при составлении сетки учитывается размер и соотношение фотографий
- Из-за добавления учета размера в сетке фотографий теперь в названии фото-файлов указывается их соотношение (`API_GET_PHOTO_ENDPOINT_URL`/`photo_id`_`width`x`height`.`format`)
- Добавлена возможность прикреплять музыкальные альбомы к постам
  - Реализовано прикрепление музыкальных альбомов при создании поста
  - Добавлена отдельная кнопка для выбора музыкальных альбомов в форме создания поста
  - В селекторе музыкальных альбомов альбомы теперь прикрепляются к посту, а не открываются на другой странице
  - Обновлена структура базы данных для хранения связей постов с музыкальными альбомами
- Добавлена возможность добавлять альбомы в библиотеку пользователя
  - Кнопка добавления альбома в библиотеку на странице альбома
  - Кнопка добавления альбома в библиотеку в компоненте альбома в списках и селекторах
  - Все треки из альбома также добавляются в библиотеку пользователя
- Практически полностью переработана страница музыки

### UI/UX fix
- Улучшен интерфейс формы создания поста:
  - Заменены отдельные кнопки для прикрепления контента одной кнопкой с выпадающим меню
  - Исправлена проблема, когда при клике на музыкальный альбом происходил переход на страницу альбома вместо его прикрепления к посту
  - Добавлена информативная иконка для кнопки прикрепления
  - Объединены функции выбора музыки и музыкальных альбомов в единый попап с вкладками
  - Улучшена навигация между разными типами музыкального контента


### Bugfix 
- Исправлен механизм пагинации на странице музыки
- Исправлена кнопка "Оригинал" на предпросмотре фотографии
- Исправлена и отредактирована форма редактирования поста
  - Исправлена ошибка из-за которой не прикреплялись приложения
  - Теперь для предпросмотра используется компонент как и в ленте для большей идентичности с реальным постом
- Также исправлено множество ошибок никак не влияющих на функционал и процесс пользования
- Исправлена проверка наличия треков и музыкальных альбомов в библиотеке пользователя

## 0.4.2
> dev time 01.05.2025 - 02.05.2025

### New Features (minor)
- Добавлен поиск на странице музыки 

### Bugfix
- Исправлено множество ошибок 401 на странице музыки связанных с переходом на JWT токены

### UI/UX fix
- Переработан компонент UniversalTrackItem для большего удобства
- Доработана витрина (Showcase)
  - Исправлено множество отступов
  - Исправлены цвета и заменена иконка при отсутствии фотографий  
- Переработана страница музыки (не до конца)
  - Все треки приведены к `UniversalTrackItem`
  - Изменен цвет (не доделано)


### Other
- Были добавлены скрипты для развертывания проекта на системах Linux (`/setup/linux`)
- Был добавлен Todo.md для отслеживания целей разработки

## 0.4.1
> dev time 30.04.2025 - 01.05.2025

### Bugfix
- Исправлены ошибки связанные с заменой метода аутентификации на JWT-токены.
  - Исправлен метод получения треков пользователя, который использовал устаревший формат аутентификации.
- Исправлена ошибка, из-за которой посты, созданные от имени пользователя, не отображались на его стене из-за отсутствия параметра `wallOwnerId` при отправке данных на сервер.

## 0.4.0
> dev time 29.04.2025 - 30.04.2025
### New features
- Полностью переработана система аутентификации
  - Была заменена аутентификация с кук на JWT токен


## 0.3.0 
> dev time 21.04.25 - 29.04.2025
