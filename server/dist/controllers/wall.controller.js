...
class WallController {
    constructor() {
        this.wallPostRepository = db_connect_1.AppDataSource.getRepository(wall_entity_1.WallPost);
    ...
    async createWallPost(req, res) {
        ...
        try {
            ...
            const wallPost = new wall_entity_1.WallPost();
            ...
            if (albums && albums.length > 0) {
                ...
                for (const albumId of albums) {
                    try {
                        ...
                        console.log(`Альбом ${album.id} имеет ${((_a = album.photos) === null || _a === void 0 ? void 0 : _a.length) || 0} фотографий`);
            ...
            if (savedPost && savedPost.albums && savedPost.albums.length > 0) {
                const responseAlbums = savedPost.albums;
                console.log(`Пост ${savedPost.id} содержит ${responseAlbums.length} альбомов в ответе`);
                for (const album of responseAlbums) {
                    console.log(`Альбом ${album.id} в ответе: photosCount=${album.photosCount}, photos.length=${((_b = album.photos) === null || _b === void 0 ? void 0 : _b.length) || 0}`);
                }
            }
