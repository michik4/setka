...
class FriendController {
    ...
    static async getFriends(req, res) {
        try {
            ...
            const user = await db_connect_1.AppDataSource.getRepository(user_entity_1.User).findOne({ where: { id: targetUserId } });
            if (!user) {
            ...
            const friendConnections = await db_connect_1.AppDataSource.query(`
                SELECT f.*, 
            ...
            const friendsArray = friendConnections.map((conn) => {
            ...
            for (const friend of uniqueFriends) {
                if (friend.avatarId) {
                    const avatar = await db_connect_1.AppDataSource.getRepository(photo_entity_1.Photo).findOne({
    ...
    static async sendFriendRequest(req, res) {
        try {
            ...
            const receiver = await db_connect_1.AppDataSource.getRepository(user_entity_1.User).findOne({ where: { id: receiverId } });
            ...
            const existingRequest = await db_connect_1.AppDataSource.getRepository(friend_request_entity_1.FriendRequest).findOne({
                where: [
            ...
            const existingFriendship = await db_connect_1.AppDataSource.getRepository(friend_entity_1.Friend).findOne({
                where: [
            ...
            await db_connect_1.AppDataSource.getRepository(friend_request_entity_1.FriendRequest).save(friendRequest);
    ...
    static async acceptFriendRequest(req, res) {
        try {
            ...
            const request = await db_connect_1.AppDataSource.getRepository(friend_request_entity_1.FriendRequest).findOne({
                where: {
                    senderId: senderId,
            ...
            await db_connect_1.AppDataSource.getRepository(friend_request_entity_1.FriendRequest).save(request);
            console.log('Статус запроса обновлен на "accepted"');
            // Создаем записи о дружбе в обе стороны
            const friendship1 = new friend_entity_1.Friend();
            friendship1.userId = currentUserId;
            friendship1.friendId = senderId;
            const friendship2 = new friend_entity_1.Friend();
            friendship2.userId = senderId;
            friendship2.friendId = currentUserId;
            console.log('Создаем записи о дружбе:', friendship1, friendship2);
            const savedFriendships = await db_connect_1.AppDataSource.getRepository(friend_entity_1.Friend).save([friendship1, friendship2]);
    ...
    static async removeFriend(req, res) {
        try {
            ...
            await db_connect_1.AppDataSource.getRepository(friend_entity_1.Friend)
                .createQueryBuilder()
                .delete()
                .where('(userId = :currentUserId AND friendId = :friendId) OR (userId = :friendId AND friendId = :currentUserId)', { currentUserId, friendId })
                .execute();
            // Проверяем, существует ли запрос на дружбу и удаляем его
            await db_connect_1.AppDataSource.getRepository(friend_request_entity_1.FriendRequest)
                .createQueryBuilder()
                .delete()
                .where('(senderId = :currentUserId AND receiverId = :friendId) OR (senderId = :friendId AND receiverId = :currentUserId)', { currentUserId, friendId })
                .execute();
     ...
     */
    static async getIncomingFriendRequests(req, res) {
        try {
            ...
            const requests = await db_connect_1.AppDataSource.getRepository(friend_request_entity_1.FriendRequest)
