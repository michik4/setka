...
async function checkPostsTracks() {
  try {
    ...
    const wallTableExists = await AppDataSource.query(`
      SELECT EXISTS (
        SELECT FROM pg_tables
        WHERE schemaname = 'public'
    ...
    const postsTracks = await AppDataSource.query('SELECT * FROM posts_tracks');
    ...
    if (wallTableExists[0].exists) {
      const wallPostsTracks = await AppDataSource.query('SELECT * FROM wall_posts_tracks');
    ...
    if (wallTableExists[0].exists) {
      const wallTableStructure = await AppDataSource.query(`
        SELECT column_name, data_type 
        FROM information_schema.columns 
    ...
    const postTrackCounts = await AppDataSource.query(`
    ...
    if (wallTableExists[0].exists) {
      const wallPostTrackCounts = await AppDataSource.query(`
    ...
    if (wallTableExists[0].exists) {
      const wallPosts = await AppDataSource.query(`
        SELECT p.id, p.content, COUNT(pt."trackId") as track_count
        FROM wall_posts p
