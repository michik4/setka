...
| Метод | URL | Описание |
|-------|-----|----------|
| GET | `/api/wall/:userId` | Получение записей со стены пользователя |
| POST | `/api/wall` | Создание новой записи на стене |
| PUT | `/api/wall/:postId` | Редактирование записи на стене |
| DELETE | `/api/wall/:postId` | Удаление записи со стены |

#### Структура записи на стене:
```typescript
interface WallPost {
    id: number;           // ID записи
    content: string;      // Текст записи
    authorId: number;     // ID автора
    wallOwnerId: number;  // ID владельца стены
    author: User;         // Объект автора
    createdAt: Date;      // Дата создания
    updatedAt: Date;      // Дата обновления
}
...
1. Получение записей со стены пользователя:
```bash
GET /api/wall/1?page=1&limit=10
...
2. Создание новой записи на стене:
```bash
POST /api/wall
Content-Type: application/json
...
3. Редактирование записи на стене:
```bash
PUT /api/wall/2
Content-Type: application/json
...
Ответ:
{
    ...
    "createdAt": "2024-03-22T10:35:00.000Z",
    "updatedAt": "2024-03-22T10:36:00.000Z"
}
```

4. Удаление записи со стены:
```bash
DELETE /api/wall/2
...
#### Права доступа:
- Создавать записи на стене может любой авторизованный пользователь
- Редактировать запись может только её автор
- Удалять запись может автор записи или владелец стены
- Просматривать записи на стене может любой авторизованный пользователь
